...
/**
    ...
    @paramlimit 每页几条 必填
    @param 医院名称 选填
    @paramhosname 医院编号 选填
    @param hoscode
    @returns
 */
export const reqGetHospitalSetList = ({ page, limit, hosname, hoscode }: GetHospitalSetListParams) => {

  const response = request.get<any, GetHospitalSetListResponse>(`/admin/hosp/hospitalSet/${page}/${limit}`, {
    params: {
      hosname,
      hoscode,
    }
...
export const reqUpdateHospital = (data: UpdateHospitalParams) => {
  // data就是put请求的请求体参数
  const resp = request.put<any, null>(`/admin/hosp/hospitalSet/update`, data);
  console.log(resp)
  return resp;
};
